...
def get_garanti_index(selected_garanti):
    ...
    return garanti_dict[selected_garanti.lower()]  # Küçük harfe çevirerek eþleþmeyi saðla
...
def run_app():
    st.title('Uygulamaya hoþgeldiniz.')

    # Kullanýcý marka seçimini yapmadan önce hangi sayýnýn hangi markaya denk geldiðini belirtelim
    st.write("1: Xiaomi")
    st.write("2: Apple")
    st.write("3: Samsung")
    st.write("4: Reeder")

    # Kullanýcý marka seçimini butonlar aracýlýðýyla yapsýn
    ...
    # Kullanýcý garanti türü seçimini yapmadan önce hangi sayýnýn hangi garanti türüne denk geldiðini belirtelim
    ...
    # Kullanýcý garanti türü seçimini butonlar aracýlýðýyla yapsýn
    ...
    selected_memory = st.number_input('Dahili Hafýza', min_value=32, max_value=256)
    st.write("Dahili Hafýza:" + str(selected_memory) + "GB")

    selected_front_cam = st.slider("Ön Kamera Çözünürlüðü:", min_value=8, max_value=50)
    st.write("Ön Kamera Çözünürlüðü:" + str(selected_front_cam) + "MP")
    ...
    selected_connectionSpeed = st.radio("Baðlantý Hýzý", ('4.5G', '5G'))
